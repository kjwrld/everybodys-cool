...
export default function App() {
  ...
  const [isSchoolBookingModalOpen, setIsSchoolBookingModalOpen] = useState(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollTop >= startReveal) {
        ...
        const progress = Math.min(1, (scrollTop - startReveal) / revealDistance);
  ...
  const handleNotificationDonation = (e: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleModalDonation = (amount: string) => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      confetti.style.backgroundColor = ['#185CFC', '#457CFD', '#729CFD', '#AFC6FE', '#F9F9F9'][Math.floor(Math.random() * 5)];
    ...
    <div className="w-full">
      ...
      <div className="h-screen w-full relative">
        ...
        <div className="h-full w-full relative">
          ...
          <div className="absolute inset-0 flex items-end justify-start pb-16 lg:pb-20">
      ...
      <div className="relative z-50">
        <Navbar onOpenSchoolBooking={() => setIsSchoolBookingModalOpen(true)} />
      </div>
      ...
      {showTopCard && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-40 w-full max-w-6xl mx-auto px-4">
          <div className="flex justify-end">
            ...
            <div className={`bg-[#185CFC] rounded-2xl p-6 border border-[#185CFC]/20 shadow-xl space-y-4 relative overflow-hidden transition-all duration-300 ${
              isDonationModalOpen ? 'opacity-75 scale-[0.98]' : 'opacity-100 scale-100'
            }`}>
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <span className="text-xs text-white/90 uppercase tracking-wide">Campaign Active Now</span>
              </div>
              
              {/* Headline */}
              <div className="space-y-2 pr-6 relative z-10">
                <h2 className="text-white">Keep Services Free for Schools</h2>
                <p className="text-white/80 text-sm leading-relaxed">
                  Help us raise funds to continue offering our programs at no cost to schools in need.
                </p>
              </div>
              ...
              >
                ...
                <span className="relative z-10 font-small text-[#4A5565] text-[16px]">
                  {isDonationModalOpen ? 'Opening...' : 'Make a Difference'}
                </span>
                <ArrowRight className={`relative z-10 w-5 h-5 text-[#4A5565] transition-transform duration-200 ${
                  isDonationModalOpen ? 'translate-x-0' : 'group-hover:translate-x-1'
                }`} />
              </button>
            </div>
            </div>
